...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("distance.xml"); // Mandatory                                         // 파일 생성 뒤에 숫자를 넣어서 패킷 포함 개수 설정
  for (uint32_t i = 0; i < wifiStaNodes.GetN (); ++i)                                                     
    {
      anim.UpdateNodeDescription (wifiStaNodes.Get (i), "sta"); // Optional                               //wifiStaNodes 에 "End" 텍스트 할당, 색깔 지정
      anim.UpdateNodeColor (wifiStaNodes.Get (i), 0, 255, 0); // Optional 색깔
      anim.UpdateNodeDescription (wifiApNodes.Get (i), "AP"); // Optional                               //wifiStaNodes 에 "End" 텍스트 할당, 색깔 지정
      anim.UpdateNodeColor (wifiApNodes.Get (i), 0, 0, 255); // Optional 색깔
    }
  Simulator::Run ();
